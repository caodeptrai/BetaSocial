export const users = [
    {
        id: '9fdc8d87-9f71-4774-bbfd-d2be06b1d9fe',
        full_name: 'Dương Văn Cao',
        user_name: 'duongcao304',
        email: 'duongvancao999@gmail.com',
        password: '12345678',
        photoURL:
            'https://scontent.fhan5-9.fna.fbcdn.net/v/t39.30808-1/308476054_1103307597057379_870838083421102790_n.jpg?stp=dst-jpg_p200x200&_nc_cat=109&ccb=1-7&_nc_sid=7206a8&_nc_ohc=Yj5tkSN8RI0AX-k1pjk&_nc_ht=scontent.fhan5-9.fna&oh=00_AfBY9LauESiHpY7z3VBh7CvuTNOVorCKmyDvVCdqIp00TQ&oe=6432A51B',
        follow: 20,
        following: 10,
        posted: 2,
        saved: 8,
    },
    {
        id: 'a2dd4921-4e4a-4b6d-8cc1-ce346a096106',
        full_name: '유라⁷',
        user_name: '_yura.0910',
        email: 'yura@gmail.com',
        password: '12345678',
        photoURL: 'https://catscanman.net/wp-content/uploads/2021/09/anh-meo-cute-de-thuong-34.jpg',
        follow: 20,
        following: 10,
        posted: 2,
        saved: 7,
    },
    {
        id: 'f2409170-a2fe-4c5f-a9c2-5682cee909ad',
        full_name: 'Kim Chi',
        user_name: '_15.61c',
        email: 'kimchi@gmail.com',
        password: '12345678',
        photoURL: 'https://img.meta.com.vn/Data/image/2021/09/20/anh-meo-che-anh-meo-bua-15.jpg',
        follow: 20,
        following: 10,
        posted: 2,
        saved: 6,
    },
    {
        id: 'b7a0a108-219f-488a-998a-41fedd4b4fdb',
        full_name: 'Lê Thị Thu Hiền',
        user_name: 'qiu.xian147',
        email: 'lehien@gmail.com',
        password: '12345678',
        photoURL: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSxjib9Qrm3QGHbmK6QLXDnRLdgjX_Ldm-gkcp2X50&s ',
        follow: 20,
        following: 10,
        posted: 2,
        saved: 6,
    },
    {
        id: 'e3cd1eb0-a7f3-4e33-a367-41f877efcaef',
        full_name: 'Tuân Phạm',
        user_name: 'tunpham0311',
        email: 'tuanpham@gmail.com',
        password: '12345678',
        photoURL:
            'https://scontent.fhan15-1.fna.fbcdn.net/v/t39.30808-1/223830169_103672262016264_2183549718606660351_n.jpg?stp=c0.12.200.200a_dst-jpg_p200x200&_nc_cat=101&ccb=1-7&_nc_sid=7206a8&_nc_ohc=qUh9ZgmoSlQAX9Kgmxw&_nc_ht=scontent.fhan15-1.fna&oh=00_AfDUHTL_nWqrl8IZONzjx77wyDetpNL75fAQ7dzOzy1tOg&oe=6432E6DE',
        follow: 20,
        following: 10,
        posted: 2,
        saved: 5,
    },
    {
        id: '0837a9d4-5641-4515-9a60-8be229c82338',
        full_name: 'Dương Văn Duy',
        user_name: 'dvd.blue',
        email: 'duongvanduy@gmail.com',
        password: '12345678',
        photoURL: 'https://scontent.fhan5-9.fna.fbcdn.net/v/t39.30808-1/335647778_684916403322673_235992240994418258_n.jpg?stp=dst-jpg_p200x200&_nc_cat=110&ccb=1-7&_nc_sid=7206a8&_nc_ohc=FWDjmh2jE54AX-uj-za&_nc_ht=scontent.fhan5-9.fna&oh=00_AfDD7edM5mGjerbMAl8DsBo4cCqlNoR1o0ziQMYNS_mhLg&oe=64319F07',
        follow: 20,
        following: 10,
        posted: 2,
        saved: 4,
    },
];

export const posts = [
    {
        post_id: 'c9dac15f-eafe-4fd5-9d8b-88452aa4fcc3',
        user_post_id: '9fdc8d87-9f71-4774-bbfd-d2be06b1d9fe',
        user_post_name: 'tunpham0311',
        user_post_avatar: 'https://st.quantrimang.com/photos/image/2020/06/19/Hinh-Nen-Meo-Ngao-38.jpg',
        status: 'where are you now?',
        imagePost: 'https://ben.com.vn/tin-tuc/wp-content/uploads/2021/12/anh-che-meo-dang-yeu-cho-may-tinh-6.jpg',
        like: 1000,
    },
    {
        post_id: '38d3f95c-552f-47d5-bea3-34b93f21218c',
        user_post_id: '9fdc8d87-9f71-4774-bbfd-d2be06b1d9fe',
        user_post_name: '_15.61c',
        user_post_avatar: 'https://img.meta.com.vn/Data/image/2021/09/20/anh-meo-che-anh-meo-bua-15.jpg',
        status: 'where are you now?',
        imagePost: 'https://haycafe.vn/wp-content/uploads/2022/03/anh-meo-cute.jpg',
        like: 430,
    },
    {
        post_id: '1194801b-2741-41fb-a505-eafb12234597',
        user_post_id: '9fdc8d87-9f71-4774-bbfd-d2be06b1d9fe',
        user_post_name: 'tunpham0311',
        user_post_avatar: 'https://st.quantrimang.com/photos/image/2020/06/19/Hinh-Nen-Meo-Ngao-38.jpg',
        status: 'where are you now?',
        imagePost:
            'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQXR-XeMzii-m8iQ3g-C81pQ8ZF7LM4te3BOw&usqp=CAU',
        like: 100,
    },
    {
        post_id: 'ce11107b-80be-4ea9-9da1-db67dbff5dbc',
        user_post_id: '9fdc8d87-9f71-4774-bbfd-d2be06b1d9fe',
        user_post_name: 'tunpham0311',
        user_post_avatar: 'https://st.quantrimang.com/photos/image/2020/06/19/Hinh-Nen-Meo-Ngao-38.jpg',
        status: 'where are you now?',
        imagePost:
            'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQXR-XeMzii-m8iQ3g-C81pQ8ZF7LM4te3BOw&usqp=CAU',
        like: 20,
    },
    {
        post_id: '21afee4c-16c8-4ed4-9739-39efd0306928',
        user_post_id: '9fdc8d87-9f71-4774-bbfd-d2be06b1d9fe',
        user_post_name: 'tunpham0311',
        user_post_avatar: 'https://st.quantrimang.com/photos/image/2020/06/19/Hinh-Nen-Meo-Ngao-38.jpg',
        status: 'where are you now?',
        imagePost:
            'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQXR-XeMzii-m8iQ3g-C81pQ8ZF7LM4te3BOw&usqp=CAU',
        like: 750,
    },
    {
        post_id: '0a12d74f-6dd6-4f7c-8ff0-064d70ee4cbc',
        user_post_id: '9fdc8d87-9f71-4774-bbfd-d2be06b1d9fe',
        user_post_name: 'tunpham0311',
        user_post_avatar: 'https://st.quantrimang.com/photos/image/2020/06/19/Hinh-Nen-Meo-Ngao-38.jpg',
        status: 'where are you now?',
        imagePost:
            'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQXR-XeMzii-m8iQ3g-C81pQ8ZF7LM4te3BOw&usqp=CAU',
        like: 750,
    },
    {
        post_id: '0d6e5bca-9fc2-4ad1-a111-ca2d52f9ce4e',
        user_post_id: '9fdc8d87-9f71-4774-bbfd-d2be06b1d9fe',
        user_post_name: 'tunpham0311',
        user_post_avatar: 'https://st.quantrimang.com/photos/image/2020/06/19/Hinh-Nen-Meo-Ngao-38.jpg',
        status: 'where are you now?',
        imagePost:
            'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQXR-XeMzii-m8iQ3g-C81pQ8ZF7LM4te3BOw&usqp=CAU',
        like: 750,
    },
    {
        post_id: '68adfd11-0a16-4ba1-a983-7dc85762c51b',
        user_post_id: '9fdc8d87-9f71-4774-bbfd-d2be06b1d9fe',
        user_post_name: 'tunpham0311',
        user_post_avatar: 'https://st.quantrimang.com/photos/image/2020/06/19/Hinh-Nen-Meo-Ngao-38.jpg',
        status: 'where are you now?',
        imagePost:
            'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQXR-XeMzii-m8iQ3g-C81pQ8ZF7LM4te3BOw&usqp=CAU',
        like: 750,
    },
];

export const messages = [
    {
        messageId: 1,
        senderId: '9fdc8d87-9f71-4774-bbfd-d2be06b1d9fe',
        senderImg:
            'https://scontent.fhan15-1.fna.fbcdn.net/v/t39.30808-1/308476054_1103307597057379_870838083421102790_n.jpg?stp=dst-jpg_p200x200&_nc_cat=109&ccb=1-7&_nc_sid=7206a8&_nc_ohc=qWAm-872ZOUAX-1SZdz&_nc_ht=scontent.fhan15-1.fna&oh=00_AfA8KyBVxM_cS1RkNBse-k-XhhU9Wi-hOPDUGg4fg6tXGg&oe=6428C1DB',
        text: 'chào bạn',
        image: '',
        time: '5 minutes ago',
    },

    {
        messageId: 2,
        senderId: 2,
        senderImg:
            'https://scontent.fhan15-1.fna.fbcdn.net/v/t39.30808-1/223830169_103672262016264_2183549718606660351_n.jpg?stp=c0.12.200.200a_dst-jpg_p200x200&_nc_cat=101&ccb=1-7&_nc_sid=7206a8&_nc_ohc=1sawiuysnawAX-2-chu&_nc_ht=scontent.fhan15-1.fna&oh=00_AfCXYrqSuBpMVqXTPiVWlsBU76D_keM8cmDbrXlM9zd1tg&oe=642CF81E',
        text: 'hi',
        image: '',
        time: '5 minutes ago',
    },
    {
        messageId: 3,
        senderId: '9fdc8d87-9f71-4774-bbfd-d2be06b1d9fe',
        senderImg:
            'https://scontent.fhan15-1.fna.fbcdn.net/v/t39.30808-1/308476054_1103307597057379_870838083421102790_n.jpg?stp=dst-jpg_p200x200&_nc_cat=109&ccb=1-7&_nc_sid=7206a8&_nc_ohc=qWAm-872ZOUAX-1SZdz&_nc_ht=scontent.fhan15-1.fna&oh=00_AfA8KyBVxM_cS1RkNBse-k-XhhU9Wi-hOPDUGg4fg6tXGg&oe=6428C1DB',
        text: 'bạn có thấy con mèo này đẹp không?',
        image: 'https://ben.com.vn/tin-tuc/wp-content/uploads/2021/12/anh-che-meo-dang-yeu-cho-may-tinh-6.jpg',
        time: '2 minutes ago',
    },
    {
        messageId: 4,
        senderId: 2,
        senderImg:
            'https://scontent.fhan15-1.fna.fbcdn.net/v/t39.30808-1/223830169_103672262016264_2183549718606660351_n.jpg?stp=c0.12.200.200a_dst-jpg_p200x200&_nc_cat=101&ccb=1-7&_nc_sid=7206a8&_nc_ohc=1sawiuysnawAX-2-chu&_nc_ht=scontent.fhan15-1.fna&oh=00_AfCXYrqSuBpMVqXTPiVWlsBU76D_keM8cmDbrXlM9zd1tg&oe=642CF81E',
        text: 'có !!',
        image: '',
        time: '1 minutes ago',
    },
    {
        messageId: 1,
        senderId: '9fdc8d87-9f71-4774-bbfd-d2be06b1d9fe',
        senderImg:
            'https://scontent.fhan15-1.fna.fbcdn.net/v/t39.30808-1/308476054_1103307597057379_870838083421102790_n.jpg?stp=dst-jpg_p200x200&_nc_cat=109&ccb=1-7&_nc_sid=7206a8&_nc_ohc=qWAm-872ZOUAX-1SZdz&_nc_ht=scontent.fhan15-1.fna&oh=00_AfA8KyBVxM_cS1RkNBse-k-XhhU9Wi-hOPDUGg4fg6tXGg&oe=6428C1DB',
        text: 'hihi mình cũng thấy vậy',
        image: '',
        time: 'just now',
    },
];

export const video = [];
